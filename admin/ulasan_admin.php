...
<body class="app">   	
  ...
  <circle cx="3.5" cy="10.5" r=".5"/>
	  ...
	  <path d="M6 0h7a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2v-1a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H6a1 1 0 0 0-1 1H4a2 2 0 0 1 2-2z"></path>
						...
						<li class="nav-item">
					        <!--//Bootstrap Icons: https://icons.getbootstrap.com/ -->
					        <a class="nav-link active" href="ulasan_admin.php">
						        <span class="nav-icon">
  ...
  <path fill-rule="evenodd" d="M.146 8.354a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L1.707 7.5H10.5a.5.5 0 0 1 0 1H1.707l2.147 2.146a.5.5 0 0 1-.708.708z"/>
    ...
    <h1 class="text-center mb-4" style="color:#5d6778">Ulasan</h1>
    ...
    <tbody>
        <?php
            ...
            // Query untuk mendapatkan data ulasan beserta buku dan username
            $query = "SELECT ulasan_buku.*, user.username, buku.judul 
                      FROM ulasan_buku 
                      JOIN user ON ulasan_buku.id_user = user.id_user 
                      JOIN buku ON ulasan_buku.id_buku = buku.id_buku";
            ...
            if (!$result) {
                echo "<tr><td colspan='6' class='cell'>Terjadi kesalahan dalam pengambilan data: " . mysqli_error($koneksi) . "</td></tr>";
            } else {
                $no = 1;

                // Loop untuk menampilkan data ulasan
                while ($row = mysqli_fetch_assoc($result)) {
                    $id_ulasan = $row['id_ulasan']; // Ambil id_ulasan untuk tiap ulasan
                    echo "<tr align='center'>";
                    echo "<td class='cell'>{$no}</td>";
                    echo "<td class='cell'>{$row['username']}</td>";
                    echo "<td class='cell'>{$row['judul']}</td>";
                    echo "<td class='cell'>{$row['ulasan']}</td>";
                    echo "<td class='cell'>{$row['rating']}</td>";

                    // Tombol Aksi untuk Admin (Misalnya Hapus Ulasan)
                    echo "<td class='cell'>
                        <button type='button' class='btn btn-danger' style='color: white' data-bs-toggle='modal' data-bs-target='#deleteModal{$id_ulasan}'>
                            Hapus
                        </button>
                    </td>";
                    ...
                    echo "
                    <div class='modal fade' id='deleteModal{$id_ulasan}' tabindex='-1' aria-labelledby='deleteModalLabel' aria-hidden='true'>
                        <div class='modal-dialog'>
                            <div class='modal-content'>
                                ...
                                <div class='modal-body'>
                                    Apakah Anda yakin ingin menghapus ulasan dari <strong>{$row['username']}</strong> untuk buku <strong>{$row['judul']}</strong>?
                                </div>
                                <div class='modal-footer'>
                                    <button type='button' class='btn btn-secondary' data-bs-dismiss='modal'>Batal</button>
                                    <form action='hapus_ulasan.php' method='POST' style='display:inline-block;'>
                ...
                if ($no === 1) {
                    echo "<tr><td colspan='6' class='cell'>Tidak ada ulasan yang tersedia.</td></tr>";
                }
            }
        ?>
    </tbody>
