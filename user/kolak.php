...
<body class="app">   	
  ...
  <path fill-rule="evenodd" d="M.146 8.354a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L1.707 7.5H10.5a.5.5 0 0 1 0 1H1.707l2.147 2.146a.5.5 0 0 1-.708.708z"/>
    ...
    <h1 class="text-center mb-4" style="color:#5d6778">Koleksi Buku yang Dipinjam</h1>
    ...
    <div class="tab-content" id="orders-table-tab-content">
        <div class="tab-pane fade show active" id="orders-all" role="tabpanel" aria-labelledby="orders-all-tab">
            ...
            if (!isset($_SESSION['id_user'])) {
                echo "<div class='alert alert-warning'>Silakan login untuk melihat koleksi buku yang dipinjam.</div>";
            } else {
                // Mengambil koleksi buku berdasarkan id_user
                $query = "SELECT b.id_buku, b.judul, b.penulis, b.kategori, b.status, b.pdf_file 
                          FROM buku b 
                ...
                if (!$result) {
                    echo "<div class='alert alert-danger'>Terjadi kesalahan dalam pengambilan data: " . mysqli_error($koneksi) . "</div>";
                } else {
                    if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            echo "<div class='col-md-4'>"; // Menggunakan kolom untuk responsif
                            echo "<div class='card mb-3' style='width: 100%;'>";
                            echo "<div class='card-body'>";
                            echo "<h5 class='card-title'>{$row['judul']}</h5>";
                            echo "<h6 class='card-subtitle mb-2 text-muted'>{$row['penulis']}</h6>";
                            echo "<p class='card-text'>Kategori: {$row['kategori']}</p>";
                            echo "<p class='card-text' style='color: " . ($row['status'] == 'tersedia' ? 'green' : 'red') . "';>Status: " . ucfirst($row['status']) . "</p>";
                            echo "<a href='user/pdf/" . urlencode($row['pdf_file']) . "' class='btn btn-info' target='_blank'>Baca</a>";
