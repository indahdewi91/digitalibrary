...
<head>
    ...
    --bs-btn-color: #000;
    --bs-btn-bg: #15a362;
    --bs-btn-border-color: #15a362;
    --bs-btn-hover-color: #000;
    --bs-btn-hover-bg: #5cb377;
    --bs-btn-hover-border-color: #15a362;
    --bs-btn-focus-shadow-rgb: 77, 130, 199;
    --bs-btn-active-color: #000;
    --bs-btn-active-bg: #5cb377;
    --bs-btn-active-border-color: #15a362;
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div class="me-2">
        <input type="text" id="search" class="form-control" placeholder="Cari ulasan..." style="width: 250px;"> <!-- Atur lebar sesuai kebutuhan -->
    </div>
    <div class="d-flex align-items-center">
        <label for="limitSelect" class="me-2">Tampilkan:</label>
        <select id="limitSelect" class="form-select" onchange="changeLimit()" style="width: 70px;"> <!-- Atur lebar sesuai kebutuhan -->
                                ...
                                <?php
                                    ...
                                    if (!$result) {
                                        ...
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            echo "<tr align='center' style='cursor: pointer;' onclick=\"window.location='pinjam.php?id_buku={$row['id_buku']}';\">";
                                            echo "<td class='cell'>{$no}</td>";
                                            echo "<td class='cell'>{$row['judul']}</td>";
                                            echo "<td class='cell'>{$row['penulis']}</td>";
                                            echo "<td class='cell'>{$row['kategori']}</td>";
                                            
                                            echo "<td class='cell'>";
                                            echo "<a href='pinjam.php?id_buku={$row['id_buku']}' class='btn btn-info' style='color: white'>Pinjam Lagi</a>";
                                            echo "</td>";
                                            echo "</tr>";
                                            $no++;
                                        }
                                        ...
                                        if ($no === 1) {
                                            echo "<tr><td colspan='6' class='cell'>Tidak ada buku yang tersedia.</td></tr>";
                                        }
                                    }
                                ?>
                            </tbody>
...
document.getElementById("search").addEventListener("input", function() {
    ...
    rows.forEach(row => {
		...
		const status = row.cells[5].textContent.toLowerCase();    // Ambil judul buku
        // Cek apakah username atau judul buku mengandung nilai pencarian
        if ( judulBuku.includes(searchValue) || penulis.includes(searchValue) || kategori.includes(searchValue) || status.includes(searchValue)) {
